...
for (let index = 0; index < picArray.length; index++) {
  ...
  pictures.innerHTML += `<article class="card">
  <h2>${picArray[index].title}</h2>
  <figure>
    <img src=${picArray[index].image.medium} alt=${picArray[index].title}>
    <figcaption>${picArray[index].caption}</figcaption>
    </figure>
    <p>${picArray[index].description}</p>
    </article > `;
}
